...
import { Search, Plus, Tag, X, Box, Ruler, Save, ArrowUp, ArrowDown, Check, Edit3, Trash2, AlertTriangle, Grip, Settings2, LayoutGrid, ShoppingBag, ChevronUp, Wifi, WifiOff } from 'lucide-react';
import ProductCard from '../components/ProductCard';
import CartSidebar from '../components/CartSidebar';
import ProductConfigModal from '../components/ProductConfigModal';
import { usePOS } from '../context/POSContext';
...
// ✅ مهم: ده المصدر اللي شغال على Vercel (مش handle_tool)
import { createTransaction } from '../services/transactions.service';
...
const POSView: React.FC = () => {
  ...
  useEffect(() => {
    if (displayCategories.length > 0) {
        if (!selectedCategory || !displayCategories.some(cat => cat.name === selectedCategory)) {
            // ✅ FIX: كان عندك setSelectedCategory(displayCategories[0]) وده غلط لأنه object مش string
  ...
  // --- POS Actions ---
  // ✅ ده الحفظ اللي يشتغل فورًا (ويب + Vercel) ويعمل fallback لو Supabase فشل
  const handleConfirmTransaction = async (shouldPrint: boolean = false) => {
    if (cart.length === 0) {
      alert('السلة فارغة');
      return;
    }
    ...
    try {
      ...
      // نجاح حتى لو Offline fallback
      alert('تم حفظ العملية بنجاح');

      // لو عندك functions في context لتفريغ السلة/طباعة استخدمها هنا (حسب مشروعك)
      // if (shouldPrint) printReceipt(result);
  ...
  const handleProductSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (!newProduct.category) {
        if (displayCategories.length > 0) newProduct.category = displayCategories[0].name;
        else return;
    }
    ...
    const productData: any = {
      name: newProduct.name,
      price: parseFloat(newProduct.price),
      category: newProduct.category || displayCategories[0].name,
      pricingMethod,
      services: finalServices
    };
  ...
  return (
    <div className="flex h-full w-full overflow-hidden relative flex-col md:flex-row">
                             ...
                             <div className="flex flex-wrap gap-2">
                                {categories.map((cat: Category) => (
                                    <div 
                                      ...
                                      onDragOver={handleCategoryDragOver}
                                      onDrop={(e) => handleCategoryDrop(e, cat.name)}
                                      onDragEnd={handleCategoryDragEnd}
                                      style={{ opacity: draggedCategory === cat.name ? 0.4 : 1 }}
                                    >
                                        {isEditMode && (
                                            <div className="absolute inset-y-0 -left-2 w-4 flex items-center justify-center pointer-events-none">
                                                <Grip size={16} className="text-gray-400" />
        ...
        <div className="flex-1 overflow-y-auto pr-1 md:pr-2 pb-32 md:pb-20">
          ...
          {filteredProducts.length === 0 && (
            <div className="h-64 flex flex-col items-center justify-center text-gray-400 dark:text-gray-600">
              <p className="text-lg">{displayCategories.length === 0 ? t('addFirstCategory') : t('noProducts')}</p>
            </div>
          )}
        </div>
      </div>
      ...
      <div className="hidden md:block h-full">
          {/* ✅ مهم: CartSidebar لازم يكون هو كمان بيستخدم services مش handle_tool (هنعدل تحت) */}
          <CartSidebar />
      </div>
